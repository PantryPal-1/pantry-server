...
class cosine_rec:
    ...
    def predict(self, input, n):
        ...
        top_results = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[
            :n
        ]
        recs = pd.DataFrame(
            columns=["recipe_name", "ingredients", "recipe_urls", "score"]
        )
        for i, ind in enumerate(top_results):
            recs.at[i + 1, "recipe_name"] = self.data["recipe_name"][ind]
            recs.at[i + 1, "ingredients"] = self.data["ingredients"][ind]
            recs.at[i + 1, "recipe_urls"] = self.data["recipe_urls"][ind]
